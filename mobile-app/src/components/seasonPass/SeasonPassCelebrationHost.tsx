 ...
 * `pending_celebrations` del /season-pass/me cacheado en HomeDataContext y se
 * ackea al cerrar. Mismo patrón que UnlockModalHost: montado global en MainApp.
 */
export function SeasonPassCelebrationHost() {
  const { t } = useTranslation();
  const { seasonPassMe, ackSeasonPassCelebrations } = useHomeData();
  ...
  const dismiss = () => {
    void ackSeasonPassCelebrations(pending.map((c) => c.assignment_id));
  };
