...
> - Calcular XP y variación de elo.
...
4. CALCULAR VARIACIÓN ELO:
   - ratio = correctas / 5
   - Si ratio >= 0.8 → elo_delta = +0.1
   - Si ratio >= 0.6 → elo_delta = +0.05
   - Si ratio >= 0.4 → elo_delta = 0
   - Si ratio < 0.4  → elo_delta = -0.05
   - Aplicar multiplicador de racha al delta positivo.
   - Actualizar players.elo_rating.

5. REGISTRAR en learning_question_log: una fila por pregunta.

6. REGISTRAR en learning_sessions: una fila con el resumen.

7. ACTUALIZAR RACHA INDIVIDUAL (learning_streaks):
...
8. ACTUALIZAR RACHAS COMPARTIDAS:
...
9. RESPONDER con resultados completos.
```
...
| # | Tarea | Detalle |
|---|-------|---------|
| 3.1 | Lógica de racha individual | Función que actualiza `learning_streaks` |
| 3.2 | Integrar racha en `POST /complete` | Llamar a 3.1 al final del flujo de completar |
| 3.3 | `GET /learning/streak` | Devuelve racha + multiplicador |
| 3.4 | `GET /learning/multiplier/:playerId` | Endpoint público para otros módulos |
| 3.5 | Rachas compartidas | CRUD + lógica de reset lazy |

### Fase 4 — Cursos

| # | Tarea | Detalle |
|---|-------|---------|
| 4.1 | `GET /learning/courses` | Lista con progreso |
| 4.2 | `GET /learning/courses/:id` | Detalle con lecciones y estado |
| 4.3 | `POST /learning/courses/:id/complete-lesson` | Marcar progreso |
