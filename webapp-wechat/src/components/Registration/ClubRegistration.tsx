...
// Marca de webapp-wechat: acento naranja en lugar del rojo de web-app.
const ACCENT = '#F18F34';
const BRAND_GRADIENT = 'linear-gradient(135deg, #F18F34 0%, #E95F32 100%)';

// ID único para cada pista. Date.now() colisiona al añadir rápido (mismo ms),
// generando IDs duplicados que hacen que update/remove afecten varias pistas.
function makeCourtId(): string {
    const cryptoObj = typeof globalThis !== 'undefined' ? (globalThis.crypto as Crypto | undefined) : undefined;
    if (cryptoObj?.randomUUID) return `c-${cryptoObj.randomUUID()}`;
...
// Las tarifas deben quedar configuradas en el alta: sin precios válidos el club
// no puede calcular el precio de las reservas y la app da error.
function validatePricing(pricing: { label: string; price: string }[]): { valid: boolean; error?: string } {
  if (!pricing?.length) return { valid: false, error: 'Configura las tarifas por franja antes de continuar' };
  const allValid = pricing.every((p) => {
    const price = Number(String(p.price ?? '').replace(',', '.').trim());
    return Number.isFinite(price) && price > 0;
  });
  if (!allValid) return { valid: false, error: 'Todas las franjas deben tener un precio mayor a 0' };
  return { valid: true };
}
...
function DarkInput({
    ...
    return (
        <div>
            <label className={labelBase} htmlFor={id}>{label}{required && <span style={{ color: ACCENT }}> *</span>}</label>
...
function DarkSelect({
    ...
    return (
        <div>
            <label className={labelBase} htmlFor={id}>{label}<span style={{ color: ACCENT }}> *</span></label>
            <div className="relative group">
                {Icon && <Icon className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/12 group-focus-within:text-white/30 transition-colors" />}
                <select
                    id={id}
        ...
        >
            <div className="relative -mx-5 sm:-mx-5 -mt-2 h-28 sm:h-32 overflow-hidden">
                ...
                <div className="absolute bottom-2 sm:bottom-3 left-4 right-4 sm:left-5 sm:right-5">
                    <span className="text-[8px] font-bold uppercase tracking-[0.15em]" style={{ color: `${ACCENT}cc` }}>NUEVO CLUB</span>
                    <h2 className="text-base sm:text-lg font-black text-white leading-tight mt-0.5">Registra tu club</h2>
                    <p className="text-[10px] text-white/30 mt-0.5">Un asesor te contactará en 24h</p>
                </div>
            </div>

            <div className="space-y-2.5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <DarkInput label="Nombre" icon={User} value={data.firstName} onChange={v => onChange({ ...data, firstName: v })} placeholder="Ej: Juan" id="reg-first" />
                    <DarkInput label="Apellidos" icon={User} value={data.lastName} onChange={v => onChange({ ...data, lastName: v })} placeholder="Ej: García López" id="reg-last" />
                </div>
                <DarkInput label="Nombre del Club" icon={Building2} value={data.clubName} onChange={v => onChange({ ...data, clubName: v })} placeholder="Ej: Padel Club Madrid" id="reg-club" />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <DarkInput label="Ciudad" icon={MapPin} value={data.city} onChange={v => onChange({ ...data, city: v })} placeholder="Ej: Madrid" id="reg-city" />
                    <DarkSelect label="País" icon={Globe} value={data.country} onChange={v => onChange({ ...data, country: v })} options={COUNTRIES} placeholder="Seleccionar opción" id="reg-country" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <DarkInput label="Teléfono" icon={Phone} value={data.phone} onChange={v => onChange({ ...data, phone: v })} placeholder="Ej: +34 600 000 000" type="tel" id="reg-phone" />
                    <DarkInput label="Email corporativo" icon={Mail} value={data.email} onChange={v => onChange({ ...data, email: v })} placeholder="ejemplo@correo.com" type="email" id="reg-email" />
                </div>
                <DarkInput label="Número de pistas" icon={Hash} value={data.numCourts} onChange={v => onChange({ ...data, numCourts: v })} placeholder="8" type="number" id="reg-courts" />
                {hasTriedNext && errors.length > 0 && (
                    <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-2.5">
                        <p className="text-[10px] font-semibold text-red-400/90 mb-1">Corrige los errores antes de continuar</p>
                ...
                <div>
                    <label className={labelBase}>Deporte <span style={{ color: ACCENT }}>*</span></label>
                    <div className="flex flex-wrap gap-2">
                        {SPORTS.map(s => {
                            const selected = data.sports.includes(s);
                            return (
                                <button
                                    key={s}
                                    type="button"
                                    ...
                                    className={`min-h-[44px] min-w-[44px] px-3 py-2 rounded-full text-[10px] font-semibold transition-all touch-manipulation ${
                                        selected ? 'text-white' : 'bg-white/[0.05] text-white/25 hover:text-white/40 active:bg-white/10'
                                    }`}
            ...
            <motion.button
                ...
                style={isValid ? { background: BRAND_GRADIENT } : {}}
            >
                Siguiente <ArrowRight className="w-3.5 h-3.5" />
            </motion.button>
            <p className="text-[8px] text-white/15 text-center">Al enviar, aceptas nuestra Política de Privacidad y Términos.</p>
        </motion.div>
    );
}
...
function ManagerPhase({
    ...
    const addCourt = () => onChange({ ...data, courts: [...data.courts, { id: makeCourtId(), name: `Pista ${data.courts.length + 1}`, type: 'cristal', covered: false, lighting: true, sport: leadData.sports[0] || 'Padel' }] });
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="space-y-4">
                ...
                <motion.div key={subStep} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.2 }}>
                    ...
                    {subStep === 'billing' && (
                        <div className="space-y-2.5">
                            ...
                            <div>
                                <label className={labelBase}>Conexión Stripe <span style={{ color: ACCENT }}>*</span></label>
            ...
            <div className="flex gap-2 pt-2 sm:pt-1">
                ...
                <button type="button" onClick={nextSub} className="flex-1 min-h-[48px] py-2.5 rounded-lg text-white text-[12px] font-semibold flex items-center justify-center gap-1.5 touch-manipulation" style={{ background: BRAND_GRADIENT }}>
                    {currentIdx < 4 ? 'Siguiente' : 'Finalizar'} <ArrowRight className="w-3.5 h-3.5" />
                </button>
            </div>
        </motion.div>
    );
...
function ActivationPhase({
    ...
    const checklist = [
        { label: 'Plan seleccionado', done: !!data.selectedPlan, key: 'plan' },
        { label: 'Stripe configurado', done: data.stripeVerified || managerData.stripeConnected, key: 'stripe' },
        { label: 'Inventario listo', done: data.inventoryReady || (managerData.courts.length > 0 && managerData.pricing.some(p => Number(p.price) > 0)), key: 'inv' },
        { label: 'Revisión de soporte', done: data.supportReview === 'approved', key: 'support' },
    ];
    ...
    const handleSubmit = async () => {
        ...
        const pricingValidation = validatePricing(managerData.pricing);
        if (!pricingValidation.valid) {
            toast.error(pricingValidation.error || 'Configura las tarifas antes de publicar');
            return;
        }
        setSubmitting(true);
        try {
            ...
            toast.error(e instanceof Error ? e.message : 'Error al enviar. Inténtalo de nuevo.');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="space-y-4">
            <div className="text-center pt-2">
                ...
                <h2 className="text-[15px] font-black text-white">Publicar club</h2>
                <p className="text-[10px] text-white/25 mt-0.5">Completa estos pasos para publicar</p>
            </div>

            <div>
                <label className={labelBase}>Elige tu plan</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-1.5">
                    {PLANS.map(plan => (
                        <button key={plan.id} type="button" onClick={() => onChange({ ...data, selectedPlan: plan.id })}
                            ...
                            {plan.popular && (
                                <span className="absolute -top-1 right-1.5 px-1.5 py-px text-[6px] font-black text-white rounded-full uppercase" style={{ background: ACCENT }}>Popular</span>
                            )}
            ...
            <div>
                <div className="flex items-center justify-between mb-1.5">
                    <label className={labelBase + ' mb-0'}>Lista de comprobación</label>
                    ...
                    {checklist.map(item => (
                        <div key={item.key} className={`flex items-center gap-2.5 px-3 py-2.5 sm:py-2 min-h-[44px] rounded-lg transition-all ${item.done ? 'bg-white/[0.04]' : 'bg-white/[0.02]'}`}>
                            ...
                            {item.key === 'support' && !item.done && (
                                <span className="ml-auto text-[7px] font-bold text-amber-400/60 bg-amber-500/8 px-1.5 py-0.5 rounded">Pendiente</span>
                            )}
                        </div>
                    ))}
                </div>
            ...
            <div className="flex gap-2 pt-2 sm:pt-0">
                ...
                <button type="button" onClick={handleSubmit} disabled={submitting} className="flex-1 min-h-[48px] py-2.5 rounded-lg text-[12px] font-semibold flex items-center justify-center gap-1.5 text-white touch-manipulation"
                    style={{ background: allDone ? 'linear-gradient(135deg, #10B981, #059669)' : BRAND_GRADIENT }}>
                    {submitting ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Enviando...</> : allDone ? <><Zap className="w-3.5 h-3.5" /> Activar club</> : <><Eye className="w-3.5 h-3.5" /> Enviar a revisión</>}
                </button>
            </div>
        </motion.div>
    );
...
function SuccessScreen({ clubName, onClose }: { clubName: string; onClose: () => void }) {
    return (
        <motion.div
        ...
        >
            ...
            <h2 className="text-lg font-black text-white">Solicitud enviada</h2>
            <p className="text-[12px] text-white/30 mt-1.5 max-w-[260px] leading-relaxed">
                <span className="font-bold text-white/60">{clubName}</span> está en revisión. Te contactaremos en 24-48h.
            </p>
            <div className="mt-5 p-3.5 rounded-lg bg-white/[0.03] text-left w-full max-w-[260px]">
                <p className="text-[8px] font-bold text-white/30 mb-2 uppercase tracking-wider">Próximos pasos</p>
                <div className="space-y-2">
                    {['Email de confirmación', 'Revisión por un asesor', 'Acceso al Manager'].map((text, i) => (
...
export function ClubRegistration() {
    ...
    return (
        <div className="min-h-screen-safe bg-auth-bg flex items-end sm:items-center justify-center p-0 sm:p-4 lg:min-h-dvh">
            <motion.div
                initial={{ y: '100%' }}
                animate={{ y: 0 }}
                transition={{ type: 'spring', damping: 30, stiffness: 300 }}
                className="relative w-full max-w-md overflow-hidden flex flex-col max-h-[92dvh] sm:max-h-[88vh] rounded-t-[20px] sm:rounded-[20px] border border-auth-border"
                style={{
                    background: '#161616',
            ...
            >
                ...
                <div className="relative z-10 flex-1 min-h-0 overflow-y-auto overflow-x-hidden modal-scroll px-4 sm:px-5 pb-5 pb-[max(1.25rem,env(safe-area-inset-bottom))]" style={{ WebkitOverflowScrolling: 'touch' }}>
                    <AnimatePresence mode="wait">
                        {phase === 0 && (
                            <LeadPhase
                                key="lead"
                        ...
                        {phase === 3 && (
                            <SuccessScreen
                                key="success"
                                clubName={leadData.clubName || 'Tu club'}
                                onClose={handleClose}
                            />
                        )}
                    </AnimatePresence>
