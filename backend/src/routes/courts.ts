...
const SELECT_LIST = 'id, created_at, club_id, name, indoor, glass_type, status';
const SELECT_ONE = 'id, created_at, club_id, name, indoor, glass_type, status';
...
router.post('/', async (req: Request, res: Response) => {
  const { club_id, name, indoor, glass_type } = req.body ?? {};
  if (!club_id || !name) {
...
router.put('/:id', async (req: Request, res: Response) => {
  const { id } = req.params;
  const { name, indoor, glass_type, status } = req.body ?? {};
